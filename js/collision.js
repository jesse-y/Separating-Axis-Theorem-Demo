...
//shapes are described as a set of vertex points in local space
//to a specific centre point.
function shape(centre, shape_points) {
	...
	this.colliding = function (other) {
		//usually you would return straight away if the first shape comparison fails, but
	...
	this.get_normals = function() {
		...
		//use the shape's vertices as points to build vectors
		for (var i = 0; i < points.length; i++) {
			...
			//each normal is calculated in world space
			...
			//find the magnitude of the vector. will be used to produce
			//unit vectors
			var vector = new vector2d(x1-x2, y1-y2);
			var mag = Math.sqrt(vector.x**2 + vector.y**2);

			//the left normal of a vector (x, y) is (y, -x)
